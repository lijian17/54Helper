/**
 * 54Helper Chrome Extension Builder By Gulp
 * @author lijian
 */
...
gulp.task('zip', () => {
    ...
    // ============压缩打包================================================
    shell.exec('cd output/ && rm -rf 54helper.zip && zip -r 54helper.zip apps/ > /dev/null && cd ../');
    let size = fs.statSync('output/54helper.zip').size;
    size = pretty(size);
