...
let AjaxDebugger = (function () {
    ...
    let analyticRequest = function (request) {
        ...
        request.getContent(function (content, encoding) {
            ...
            let header = "Ajax请求加载完毕 (" + [requestPath, responseStatus, responseTime, responseSize].join(" - ") + ") " + ' -- By 54Helper';

            C54HelperConsole.group(header);
            C54HelperConsole.log('AjaxURL  :', {url: url + queryString});
            C54HelperConsole.log("Request  :", {request: request.request});
            C54HelperConsole.log("Response :", {response: request.response});
            C54HelperConsole.log("OtherInfo:", {
                timeConsuming: responseTime,
                timings: request.timings,
                time: request.startedDateTime,
                server: request.serverIPAddress
            });
            C54HelperConsole.groupEnd();
        });
    };
